...
import { CASINO_LIST, PROFILE_AVATARS, parseNameFromUrl } from './constants';
...
const getReadyTimestamp = (casino: Casino): number => {
  if (!casino.isSignedUp || !casino.hasDailyBonus) return Infinity;
  ...
  if (config.type === 'rolling') {
    ...
    return lastClaim + cooldownMs;
  } else {
    // Fixed reset time logic
    ...
    let lastReset = new Date(todayReset);
    if (now < lastReset.getTime()) {
         lastReset.setDate(lastReset.getDate() - 1);
    }

    if (lastClaim < lastReset.getTime()) {
        return 0; // Ready now (it was ready at lastReset, and hasn't been claimed since)
    }
    
    // Next reset is 24h after the last reset
...
const isBonusReady = (casino: Casino): boolean => {
  return getReadyTimestamp(casino) <= Date.now();
};
...
export const App: React.FC = () => {
  ...
  const casinos = useMemo(() => {
    const systemList = CASINO_LIST.map((c, index) => {
  ...
  const filteredCasinos = useMemo(() => {
    ...
    if (activeFilter === 'daily' || activeFilter === 'focus') {
        list.sort((a, b) => {
             const timeA = getReadyTimestamp(a);
             const timeB = getReadyTimestamp(b);
             const isReadyA = timeA <= now;
             const isReadyB = timeB <= now;
             
             // 1. Ready first
             if (isReadyA && !isReadyB) return -1;
             if (!isReadyA && isReadyB) return 1;

             // 2. Both ready or both not ready: sort by time remaining (ascending)
             // (Ready ones will have time <= now, often 0 or negative relative to now if logic changes, but getReadyTimestamp handles it)
             if (timeA !== timeB) return timeA - timeB;
             
             // 3. Fallback: Signed Up status (for Daily tab which shows all daily enabled)
    ...
    return list;
  }, [casinos, searchQuery, activeFilter, filterScope, now]);
  ...
  if (showGlobalConfig) {
     const allCasinosForConfig = CASINO_LIST.map((c, index) => {
      const id = `casino-${index}`;
      const name = c.name;
      const url = c.url;
      const shared = globalSettings[id] || { hasDailyBonus: false, hasReferral: false, bonusConfig: undefined };
